...
pub fn invoke_validate_stat<'info>(
    ...
    data.extend(
        args.try_to_vec()
            .map_err(|_| error!(TxLineCpiError::SerializationFailed))?,
    );
    ...
    .map_err(|_| error!(TxLineCpiError::ValidateStatFailed))?;
...
pub enum TxLineCpiError {
    #[msg("Failed to serialize validate_stat CPI args")]
    SerializationFailed,
    #[msg("TxLINE validate_stat CPI failed")]
    ValidateStatFailed,
}
